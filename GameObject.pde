public class GameObject {
  ...
  boolean isCollidingWith(GameObject other) {
    return (r.x - s.x / 2 <= other.r.x + other.s.x / 2
         && r.x + s.x / 2 >= other.r.x - other.s.x / 2
         && r.y - s.y / 2 <= other.r.y + other.s.y / 2
         && r.y + s.y / 2 >= other.r.y - other.s.y / 2);
  }

  CollisionDirection directionOfCollisionWith(GameObject other) {
    float nx = max(other.r.x - other.s.x / 2, r.x - s.x / 2);
    float ny = max(other.r.y - other.s.y / 2, r.y - s.y / 2);
    float nWidth = min(r.x + s.x / 2, other.r.x + other.s.x / 2) - nx;
